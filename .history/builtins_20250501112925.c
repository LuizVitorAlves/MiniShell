...
void ft_unset(char *arg)
{
    ...
    while (env_vars[i])
    {
        // Encontrar '=' para limitar comparação ao nome da variável
        char *equal_sign;
        
        equal_sign = ft_strchr(env_vars[i], '=');
        if (equal_sign)
        {
            int var_name_len = equal_sign - env_vars[i];
            if (ft_strlen(arg) == var_name_len &&
                ft_strncmp(env_vars[i], arg, var_name_len) == 0)
            {
                // Libera a variável e move todas as seguintes para frente
                free(env_vars[i]);
                j = i;
                while (env_vars[j])
                {
                ...
                continue; // Não incrementa i, pois agora há um novo valor em env_vars[i]
            }
        }
        i++;
    }
